...
def toggle_webcam():
    ...
    if iswebcam:
        ...
        if camera is None or not camera.isOpened():
            camera = Detector(videoPath=0, modelPath=model_path, target_video_width=target_width)
...
def upload_file():
    ...
    if file and allowed_file(file.filename):
        ...
        if camera is not None and camera.cam.isOpened():
            camera.cam.release()
        camera = Detector(videoPath=filepath, modelPath=model_path, target_video_width=target_width)
        target_width = camera.target_width
        target_height = camera.target_height
